...
export function ServiciosSection({
  ...
  const [servicioActual, setServicioActual] = useState<
    ...
    >
  >({});
  const [servicioModalOpen, setServicioModalOpen] = useState<boolean>(false);
  ...
  const fetchServicios = useCallback(async () => {
    try {
      ...
      if (searchTerm.trim()) {
        response = await ServiciosService.buscarServiciosDePerfil(
          perfilId,
          searchTerm,
        ...
        response = await ServiciosService.getServiciosDePerfil(
          perfilId,
          (currentPage - 1) * pageSize,
          pageSize
        );
  ...
  const debouncedFetchServicios = useMemo(
    () => debounce(fetchServicios, 300),
    [fetchServicios]
  );

  useEffect(() => {
    debouncedFetchServicios();
    return () => {
      debouncedFetchServicios.cancel();
    };
  }, [debouncedFetchServicios]);
  ...
  const validateForm = (): boolean => {
    ...
    if (!servicioActual.nombre?.trim()) {
      errors.nombre = 'El nombre es requerido';
      isValid = false;
    }

    if (!servicioActual.detalle?.trim()) {
      errors.detalle = 'El detalle es requerido';
      isValid = false;
    } else if (servicioActual.detalle.length > 255) {
      errors.detalle = 'El detalle no puede tener más de 255 caracteres';
      isValid = false;
    }

    if (!servicioActual.caracteristicas?.descripcion?.trim()) {
      errors.descripcionCaracteristicas =
        'La descripción de características es requerida';
      isValid = false;
    } else if (servicioActual.caracteristicas.descripcion.length > 255) {
      errors.descripcionCaracteristicas =
        'La descripción de características no puede tener más de 255 caracteres';
      isValid = false;
    }

    if (!servicioActual.caracteristicas?.items?.length) {
    ...
    if (
      !modoEdicion &&
      (!servicioActual.imagenes || servicioActual.imagenes.length === 0)
    ) {
  ...
  const agregarServicio = async (e: FormEvent) => {
    e.preventDefault();
    if (!validateForm()) return;

    try {
      setIsLoading(true);
      const createServicioDto: CreateServicioDto = {
        nombre: servicioActual.nombre!,
        detalle: servicioActual.detalle!,
        caracteristicas: servicioActual.caracteristicas!,
        imagenes: servicioActual.imagenes || [],
        perfilId: perfilId,
        subcategoriaId: selectedSubcategoria!
      };

      await ServiciosService.createServicio(createServicioDto);
      setServicioActual({});
  ...
  const editarServicio = async (e: FormEvent) => {
    e.preventDefault();
    if (!validateForm()) return;
    ...
    try {
      setIsLoading(true);
      const updateServicioDto: CreateServicioDto = {
        nombre: servicioActual.nombre!,
        detalle: servicioActual.detalle!,
        caracteristicas: servicioActual.caracteristicas!,
        imagenes: servicioActual.imagenes || [],
        perfilId: perfilId,
        subcategoriaId: selectedSubcategoria!,
        imagenesEliminadas: servicioActual.imagenesEliminadas || []
      };
  ...
  const abrirEditarServicio = async (servicio: Servicio) => {
    setServicioActual({
      ...servicio,
      imagenesEliminadas: []
    });
    ...
    if (servicio.subcategoria) {
      const categoria = servicio.subcategoria.categoria;
      ...
      setSubcategorias(subcategoriasData);
      setSelectedSubcategoria(servicio.subcategoria.id);
    }
  };
  ...
  const handleServicioChange = (
    ...
    if (id === 'descripcionCaracteristicas') {
      setServicioActual({
        ...servicioActual,
        caracteristicas: {
          ...(servicioActual.caracteristicas || { items: [], descripcion: '' }),
          descripcion: value
        }
      });
    } else if (id === 'imagenes') {
      ...
      setServicioActual({ ...servicioActual, [id]: value });
    }
  };
  ...
  const agregarItemCaracteristica = () => {
    ...
    setServicioActual({
      ...servicioActual,
      caracteristicas: {
        ...(servicioActual.caracteristicas || { descripcion: '' }),
        items: [
          ...(servicioActual.caracteristicas?.items || []),
  ...
  const eliminarItemCaracteristica = (index: number) => {
    setServicioActual({
      ...
      caracteristicas: {
        ...(servicioActual.caracteristicas || { descripcion: '' }),
        items: (servicioActual.caracteristicas?.items || []).filter(
          (_, i) => i !== index
        )
      }
    });
    ...
    <>
      <div className="bg-white/30 rounded-lg overflow-hidden">
        <div className="flex justify-between items-center border-b px-4">
          ...
          >
            <span className="mr-2">+</span> Nuevo Servicio
          </button>
        </div>
                ...
                servicios.map((servicio) => (
                  <tr key={servicio.id}>
                    <td className="px-4 py-4 whitespace-nowrap text-white">
                      <div className="flex items-center gap-2">
                        {servicio.imagenes &&
                          ...
                          typeof servicio.imagenes[0] === 'string' && (
                            <Image
                    ...
                    {/* <td className="px-4 py-4 whitespace-nowrap hidden md:table-cell text-white">
                      {servicio.detalle.length > 50
                        ? `${servicio.detalle.substring(0, 50)}...`
                        : servicio.detalle}
                    </td> */}
                    <td className="px-4 py-4 whitespace-nowrap hidden md:table-cell text-white">
      ...
      >
        <DialogContent className="max-w-md md:max-w-xl max-h-[90vh] overflow-y-auto bg-white/30 backdrop-blur-md">
          ...
          <form
            onSubmit={modoEdicion ? editarServicio : agregarServicio}
            className="space-y-4"
          >
            <div>
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">
                {servicioActual.detalle?.length || 0}/255 caracteres
              </p>
            </div>
            ...
            <div>
              ...
              <Input
                ...
                isTextArea
                value={servicioActual.caracteristicas?.descripcion || ''}
                onChange={handleServicioChange}
                rows={2}
                className={`text-white ${
              ...
              <p className="text-gray-400 text-sm mt-1">
                {servicioActual.caracteristicas?.descripcion?.length || 0}/255
                caracteres
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2 mt-2">
                {(servicioActual.caracteristicas?.items || []).map(
                  (item, index) => (
                    <div
                      key={index}
            ...
            <div className="space-y-1">
              ...
              <ImageDropzone
                images={servicioActual.imagenes || []}
                maxSizeBytes={350 * 1024}
                isEditMode={modoEdicion}
                onChange={(newImages) => {
                  const imagenesEliminadas = (servicioActual.imagenes || [])
                    .filter(
                      (img) =>
                  ...
                  setServicioActual({
                    ...
                    imagenesEliminadas: [
                      ...(servicioActual.imagenesEliminadas || []),
                      ...imagenesEliminadas
                    ]
                  });
                }}
